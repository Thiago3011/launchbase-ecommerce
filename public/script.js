const Mask = {
  apply (input, func) {
    setTimeout(function () {
      input.value = Mask[func](input.value) // Mask[func](input.value) == Mask.func(input.value)
    }, 1)
  },
...
const PhotosUpload = {
  ...
  hasLimit (event) {
    ...
    if (fileList.length > uploadLimit) {
      window.alert(`Envie no Máximo ${uploadLimit} fotos`)
  ...
  getAllFiles () {
    const dataTransfer = new window.ClipboardEvent('').clipboardData || new window.DataTransfer()
